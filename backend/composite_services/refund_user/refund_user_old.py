...
#     # Step 2: Extract ticket price from order data
#     try:
#         # Look for ticket price information in the order
#         # The exact structure depends on your Order Service's data model
#         # Option 1: If order has ticket-specific prices
#         if "ticketPrices" in order_data and isinstance(order_data["ticketPrices"], dict):
#             refund_amount = order_data["ticketPrices"].get(ticket_id)
#         # Option 2: If order has items array with ticket details
#         elif "items" in order_data and isinstance(order_data["items"], list):
#             ticket_item = next((item for item in order_data["items"] 
#                                if item.get("ticketId") == ticket_id), None)
#             refund_amount = ticket_item.get("price") if ticket_item else None
#         # Option 3: If order has a simple total price that should be divided by number of tickets
#         elif "totalPrice" in order_data and "ticketIds" in order_data and isinstance(order_data["ticketIds"], list):
#             total_tickets = len(order_data["ticketIds"])
#             if total_tickets > 0:
#                 refund_amount = float(order_data["totalPrice"]) / total_tickets
#             else:
#                 refund_amount = None
#         else:
#             # Fallback option if order structure doesn't contain ticket price info
#             logger.error(f"Order doesn't contain ticket price information: {order_data}")
#             return jsonify({
#                 "error": "Unable to determine refund amount from order",
#                 "order_keys": list(order_data.keys())
#             }), 400
        
#         if not refund_amount:
#             return jsonify({"error": "Ticket price not found in order"}), 400
            
#         refund_amount = round(float(refund_amount), 2)
#         if refund_amount <= 0:
#             return jsonify({"error": "Refund amount must be positive"}), 400
#     except (ValueError, KeyError) as e:
#         logger.error(f"Error extracting refund amount: {str(e)}")
#         return jsonify({"error": f"Invalid ticket price data: {str(e)}"}), 400
...
#     # Step 6: Remove ticket from order
...
#         message=f"A refund of ${refund_amount:.2f} for ticket {ticket_id} has been processed",
